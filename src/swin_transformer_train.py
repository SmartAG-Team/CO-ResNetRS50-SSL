...
def main(args):
    ...
    # 修改分类头以适应你的任务（例如，7 类分类任务）
    model.head = torch.nn.Linear(model.head.in_features, 7).to(device)
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/SwinTransformer/result.csv", "w") as f:
        f.write("Epoch,Train_Loss,Train_Acc,Val_Loss,Val_Acc,Test_Loss,Test_Acc,Test_Pre,Test_Recall,Test_F1,Test_1_Acc,Test_2_Acc,Test_3_Acc,Test_4_Acc,Test_5_Acc,Test_7_Acc,Test_8_Acc,Test_Confusion_Matrix,Model_Parameter\n")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch < args.epochs:
                f.write(f"{epoch},{average_train_loss:.4f},{average_train_accuracy:.4f},{average_val_loss:.4f},{average_val_accuracy:.4f},{None},{None},{None},{None},{None},{None},{None},{None},{None},{None},{None},{None},{None},{total_params:.2f}\n")
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/SwinTransformer/' + filename + 'ckpt.t7')
                best_val_accuracy = average_val_accuracy

        # 测试模型
        val_checkpoint = torch.load('./model/SwinTransformer/Best_model_ckpt.t7')
