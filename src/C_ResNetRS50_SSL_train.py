...
from model.C_ResNetRS50_SSL.ResNetRS import ResNet
...
def main(args):
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/C_ResNetRS50_SSL/result.csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/C_ResNetRS50_SSL/' + filename + 'ckpt.t7')
        ...
        # 测试模型
        val_checkpoint = torch.load('./model/C_ResNetRS50_SSL/Best_model_ckpt.t7')
