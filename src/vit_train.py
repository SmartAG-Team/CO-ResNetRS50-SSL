...
def main(args):
    ...
    model.heads.head = torch.nn.Linear(model.heads.head.in_features, 7).to(device)
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/vit/result.csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/vit/' + filename + 'ckpt.t7')
                best_val_accuracy = average_val_accuracy

        # 测试模型
        val_checkpoint = torch.load('./model/vit/Best_model_ckpt.t7')
