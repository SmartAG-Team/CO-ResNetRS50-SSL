...
from model.FasterNet_t1.FasterNet import FasterNet
...
def main(args):
    ...
    model = FasterNet(num_classes=7).to(device)
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/FasterNet_t1/result.csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/FasterNet_t1/' + filename + 'ckpt.t7')
                best_val_accuracy = average_val_accuracy

        # 测试模型
        val_checkpoint = torch.load('./model/FasterNet_t1/Best_model_ckpt.t7')
