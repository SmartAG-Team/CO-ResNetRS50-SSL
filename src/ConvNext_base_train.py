...
from model.ConvNext.convnext import convnext_base
...
def main(args):
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/ConvNext/result.csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/ConvNext/' + filename + 'ckpt.t7')
                best_val_accuracy = average_val_accuracy

        # 测试模型
        val_checkpoint = torch.load('./model/ConvNext/Best_model_ckpt.t7')
