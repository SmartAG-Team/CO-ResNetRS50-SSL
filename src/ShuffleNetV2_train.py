...
def main(args):
    ...
    model = models.shufflenet_v2_x1_0(num_classes=7).to(device)
    ...
    # 新增：创建CSV文件并写入列标题
    with open("./data/ShuffleNetV2/result.csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_val_accuracy < average_val_accuracy:
                ...
                torch.save(state, './model/ShuffleNetV2/' + filename + 'ckpt.t7')
                best_val_accuracy = average_val_accuracy

        # 测试模型
        val_checkpoint = torch.load('./model/ShuffleNetV2/Best_model_ckpt.t7')
